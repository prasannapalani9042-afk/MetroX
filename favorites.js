...
function renderFavs(){
  const f=getFavs();
  $('#favList').innerHTML = f.length ? f.map(n=>{
    const ln=lineOf(n)[0];
    return `<div class="stationline"><span class="dot" style="background:${LINES[ln].color}"></span>${n}
      <span class="mini">(${ln} Line)</span>
      <span style="margin-left:auto;cursor:pointer" onclick="toggleFav('${n}');renderFavs()">❤️</span></div>`;
  }).join('') : '<div class="empty">No favorite stations yet. Add some from Metro Lines →</div>';
}
