...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#timeForm').onsubmit=e=>{
    ...
    const stops=stopCount(path), tm=timeFor(stops,path.interchange);
    $('#timeResult').innerHTML=`<span class="badge" style="background:rgba(77,124,255,.12);color:var(--blue)">TIME</span>
      <h2 style="margin:10px 0">${tm} min</h2>
      <p>${f.value} → ${t.value} · ${nodeCount(path)} stations · avg 2.5 min/stop ${path.interchange?'+ 4 min interchange wait':''}</p>`;
    pushHistory({from:f.value,to:t.value,type:tm+' min',ts:Date.now()});
  };
});
