...
document.addEventListener('DOMContentLoaded',()=>{
  function fallbackList(){
    const uniq=[...new Map(allStations().map(s=>[s.name,s])).values()];
    const hub=uniq.find(s=>s.name==='Chennai Central')||uniq[0];
    const near=uniq.map(s=>({...s,d:haversine(hub,s)})).sort((a,b)=>a.d-b.d).slice(0,5);
    return near.map(s=>`<div class="stationline"><span class="dot" style="background:${LINES[s.line].color}"></span>${s.name}
      <span class="badge" style="background:${LINES[s.line].color}22;color:${LINES[s.line].color};margin-left:8px">${s.line}</span></div>`).join('');
  }
  $('#nearbyBtn').onclick=()=>{
    const box=$('#nearbyResult'); box.innerHTML='<div class="empty">Locating you…</div>';
    if(!('geolocation' in navigator)){
      box.innerHTML='<div class="empty">This browser does not support location. Showing stations near Chennai Central instead.</div>'+fallbackList();
      return;
    }
    const insecure = location.protocol==='file:' || (location.protocol==='http:' && !['localhost','127.0.0.1'].includes(location.hostname));
    if(insecure){
      box.innerHTML='<div class="empty">📍 Mobile browsers block GPS on file:// pages for security. Host these files on a server (or open over HTTPS) to use your live location. Showing stations near Chennai Central instead.</div>'+fallbackList();
      return;
    }
    navigator.geolocation.getCurrentPosition(pos=>{
      const me={lat:pos.coords.latitude,lng:pos.coords.longitude};
      const uniq=[...new Map(allStations().map(s=>[s.name,s])).values()];
      const near=uniq.map(s=>({...s,d:haversine(me,s)})).sort((a,b)=>a.d-b.d).slice(0,5);
      box.innerHTML=near.map(s=>`<div class="stationline"><span class="dot" style="background:${LINES[s.line].color}"></span>${s.name}
        <span class="badge" style="background:${LINES[s.line].color}22;color:${LINES[s.line].color};margin-left:8px">${s.line}</span>
        <span style="margin-left:auto;color:var(--muted)">${s.d.toFixed(1)} km</span></div>`).join('');
    },err=>{
      let msg='Could not get your location.';
      if(err.code===1) msg='Location permission denied. Please allow location access for this site in your browser settings.';
      else if(err.code===2) msg='Location unavailable. Please check your device GPS/network and try again.';
      else if(err.code===3) msg='Location request timed out. Please try again.';
      box.innerHTML=`<div class="empty">${msg}</div>`+fallbackList();
