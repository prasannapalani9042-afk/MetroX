...
document.addEventListener('DOMContentLoaded',()=>{
  const a=$('#from'), b=$('#to');
  uniqueNames().forEach(s=>{a.add(new Option(s,s));b.add(new Option(s,s))});
  $('#routeForm').onsubmit=e=>{
    e.preventDefault();
    const path=findPath(a.value,b.value);
    if(!path){toast('Choose two different stations.');return;}
    const total=nodeCount(path), stops=stopCount(path);
    $('#result').innerHTML=`<span class="badge" style="background:rgba(66,230,164,.12);color:var(--success)">${total} STATIONS</span>
      <h2 style="margin:10px 0">${a.value} → ${b.value}</h2>
      <p>Fare ₹${fareFor(stops)} · ~${timeFor(stops,path.interchange)} min ${path.interchange?'· 1 interchange':''}</p>
      ${renderPathHTML(path,total)}`;
    pushHistory({from:a.value,to:b.value,type:total+' stations',ts:Date.now()});
  };
});
