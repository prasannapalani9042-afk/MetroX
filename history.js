...
function renderHistory(){
  const h=getHistory();
  $('#histList').innerHTML = h.length ? h.map(e=>`<div class="stationline"><span class="dot" style="background:var(--blue)"></span>${e.from} → ${e.to}
    <span style="margin-left:auto;color:var(--muted);font-size:11.5px">${e.type}</span></div>`).join('') : '<div class="empty">No journeys yet. Try the Route Finder!</div>';
}
document.addEventListener('DOMContentLoaded',()=>{
  renderHistory();
  $('#clearBtn').onclick=()=>{clearHistory();renderHistory();};
});
