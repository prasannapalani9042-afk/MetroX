...
function renderLines(){
  $('#lineInfo').innerHTML = Object.entries(LINES).map(([ln,d])=>`
  <div class="card result-box" style="margin-top:16px">
    <div style="display:flex;justify-content:space-between;align-items:center">
      <span class="badge" style="background:${d.color}22;color:${d.color};font-size:12px;padding:6px 12px">${ln} Line</span>
      <span class="badge" style="background:${d.color}22;color:${d.color}">${lineStations(ln).length} stations</span>
    </div>
    <div style="margin-top:12px">${lineStations(ln).map(s=>`
      <div class="stationline" style="cursor:pointer" onclick="toggleFav('${s}');renderLines()">
        <span class="dot" style="background:${d.color}"></span>${s}
        ${isInterchange(s)?'<span style="color:var(--orange);font-size:11px">⇄ interchange</span>':''}
        <span style="margin-left:auto">${isFav(s)?'❤️':'🤍'}</span>
      </div>`).join('')}
    </div>
  </div>`).join('');
}
document.addEventListener('DOMContentLoaded',renderLines);
