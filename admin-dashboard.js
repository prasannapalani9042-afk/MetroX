...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  $('#adminForm').onsubmit=e=>{
    e.preventDefault();
    const name=$('#stnName').value.trim();
    if(!name){ toast('Enter a station name.'); return; }
    let a=getAdminStations();
    a.push({name,line:$('#stnLine').value});
    saveAdminStations(a);
    $('#stnName').value='';
    renderAdmin();
    toast('Station added — it now appears across the whole app.');
  };
});
function renderAdmin(){
  const a=getAdminStations();
  let rows='<tr><th>Station</th><th>Line</th><th></th></tr>';
  rows+=a.map((s,i)=>`<tr><td>${s.name}</td><td><span class="badge" style="background:${LINES[s.line].color}22;color:${LINES[s.line].color}">${s.line}</span></td>
    <td><span style="cursor:pointer;color:var(--danger)" onclick="adminDel(${i})">Remove</span></td></tr>`).join('');
  if(!a.length) rows+='<tr><td colspan="3" style="color:var(--muted);text-align:center">No custom stations added yet</td></tr>';
  $('#adminTable').innerHTML=rows;
}
function adminDel(i){ let a=getAdminStations(); a.splice(i,1); saveAdminStations(a); renderAdmin(); }
