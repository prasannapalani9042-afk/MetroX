...
document.addEventListener('DOMContentLoaded',()=>{
  const s=$('#statStations'); if(s) s.textContent=uniqueNames().length;
  const f=$('#statFav'); if(f) f.textContent=getFavs().length;
  const h=getHistory();
  const rec=$('#homeRecent');
  if(rec) rec.textContent = h.length ? `${h[0].from} → ${h[0].to} (${h[0].type})` : 'No recent journeys yet';
});
